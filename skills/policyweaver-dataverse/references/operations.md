...
For `role_naming: "readable"`, first follow the watchdog-first rollout in `docs/READABLE-ROLES-AND-DIVERSITY.md`. Fresh source label discovery records all observed direct/team role origins and effective Entra role results. The readable primary business role is a summary; it is not a one-to-one Dataverse-to-Fabric role copy or an authorization input. Review the manifest's `reader_labels` and shard `role_name_map`. The default legacy mode preserves prior configuration hashes; switching modes requires a new generation. A label-discovery error blocks readable preparation rather than dropping a role silently.
